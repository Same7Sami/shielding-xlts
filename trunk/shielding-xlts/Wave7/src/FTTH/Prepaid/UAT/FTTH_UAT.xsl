	...
	<xsl:template match="/">
		<TCRMService xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
			...
			<RequestControl>
				...
				<DWLControl>
					<requesterName>FTTHUatPre</requesterName>
			...
			<TCRMTx>
				...
				<ContractCompositeMBObj>
							<TCRMContractBObj>
								...
								<xsl:if test="//ServiceStartDate and //ServiceStartDate!=''">
								<LastVerifiedDate>
										<xsl:call-template name="formatDate">
											<xsl:with-param name="str" select="//ServiceStartDate" />
										</xsl:call-template>
								</LastVerifiedDate>
								</xsl:if>
								<TCRMExtension>
									...
									<XContractBObjExt>
										...
										<xsl:if test="//ExpDate and //ExpDate!=''">
										<ExpiryDate>
										<xsl:call-template name="formatDate">
											<xsl:with-param name="str" select="//ExpDate" />
										</xsl:call-template>
										</ExpiryDate>
										</xsl:if>
									</XContractBObjExt>
