<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform">

	<xsl:output method="xml" indent="yes"/>
	<xsl:template match="/">
		<TCRMService xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
			xsi:noNamespaceSchemaLocation="mobilyRequest.xsd">
			<RequestControl>
				<requestID>
				<xsl:call-template name="recurse-over-string">
							<xsl:with-param name="str" select="//MsgId"/>
				</xsl:call-template>
				</requestID>
				<DWLControl>
				<xsl:choose>
				<xsl:when test="//ServiceId = '306' or //ServiceId = '308' or //ServiceId = '309'">
					<requesterName>FirstCall</requesterName>
				</xsl:when>
				<xsl:when test="//ServiceId = '307'">
					<requesterName>EndofLifeCycle</requesterName>
				</xsl:when>
				<xsl:otherwise>
					<requesterName>LifeCycleChange</requesterName>
				</xsl:otherwise>
				</xsl:choose>
					<requesterLanguage>100</requesterLanguage>
				</DWLControl>
			</RequestControl>
			<TCRMTx>
				<TCRMTxType>updateContractComposite</TCRMTxType>
				<TCRMTxObject>ContractCompositeBObj</TCRMTxObject>
				<TCRMObject>
				<ContractCompositeBObj>
				<ContractCompositeMBObj>
							<TCRMContractBObj>
							<xsl:choose>
								<xsl:when test="//ServiceId = '307'">
									<EndDate>
									<xsl:call-template name="formatDate">
											<xsl:with-param name="str" select="//LifeCycleEndDate"/>
									</xsl:call-template>
									</EndDate>
									<AgreementStatusType>4</AgreementStatusType>
								</xsl:when>
								<xsl:when test="//ServiceId = '306' or //ServiceId = '308' or //ServiceId = '309'">
									<AgreementStatusType>5</AgreementStatusType>
									<xsl:if test="//LifeCycleStartDate and //LifeCycleStartDate!=''">
										<LastVerifiedDate>
											<xsl:call-template name="formatDate">
												<xsl:with-param name="str" select="//LifeCycleStartDate"/>
											</xsl:call-template>
										</LastVerifiedDate>
									</xsl:if>
								</xsl:when>
							</xsl:choose>
								<TCRMExtension>
									<ExtendedObject>XContractBObjExt</ExtendedObject>
									<XContractBObjExt>
									<xsl:choose>
										<xsl:when test="//ServiceId = '304'">
										<NetworkStatusType>3</NetworkStatusType>
										<ChargingStatusType>2</ChargingStatusType>
										</xsl:when>
										<xsl:when test="//ServiceId = '305'">
										<NetworkStatusType>4</NetworkStatusType>
										<ChargingStatusType>3</ChargingStatusType>
										</xsl:when>
										<xsl:when test="//ServiceId = '306' or //ServiceId = '308' or //ServiceId = '309'">
										<NetworkStatusType>1</NetworkStatusType>
										<ChargingStatusType>1</ChargingStatusType>
										</xsl:when>
										<xsl:when test="//ServiceId = '307'">
										<NetworkStatusType>6</NetworkStatusType>
										<ChargingStatusType>5</ChargingStatusType>
										</xsl:when>
									</xsl:choose>
									</XContractBObjExt>
								</TCRMExtension>
							<xsl:choose>
								<xsl:when test="//ServiceId = '307'">
								<TCRMContractComponentBObj>
									<ContractStatusType>6</ContractStatusType>
								</TCRMContractComponentBObj>
								</xsl:when>
								<xsl:when test="//ServiceId = '306' or //ServiceId = '308' or //ServiceId = '309'">
								<TCRMContractComponentBObj>
									<ContractStatusType>7</ContractStatusType>
								</TCRMContractComponentBObj>
								</xsl:when>
							</xsl:choose>
							</TCRMContractBObj>
					<ContractSearchParamsBObj>
						<MSISDN>
						<xsl:call-template name="msisdnFormat">
								<xsl:with-param name="str" select="//SubscriberId"/>
						</xsl:call-template>
                    	</MSISDN>
                 	</ContractSearchParamsBObj>
				</ContractCompositeMBObj>
				</ContractCompositeBObj>
				</TCRMObject>
			</TCRMTx>
		</TCRMService>
		
	</xsl:template>
	<xsl:template name="recurse-over-string">
		<xsl:param name="str"/>
		<xsl:if test="$str">
			<xsl:variable name="first-char" select="substring($str,1,1)"/>
			<xsl:choose>
				<xsl:when
					test="$first-char = '0' or $first-char = '1'or $first-char = '2'or $first-char = '3'or $first-char = '4'or $first-char = '5'or $first-char = '6'or $first-char = '7'or $first-char = '8'or $first-char = '9'">
					<xsl:value-of select="$first-char"/>
				</xsl:when>
			</xsl:choose>
			<xsl:if test="string-length($str) > 1">
				<xsl:call-template name="recurse-over-string">
					<xsl:with-param name="str" select="substring($str,2)"/>
				</xsl:call-template>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	
	<xsl:template name="msisdnFormat">
		<xsl:param name="str"/>
		<xsl:if test="$str">966<xsl:value-of select="substring($str,2)"/></xsl:if>
	</xsl:template>
	

<xsl:template name="formatDate">
		<xsl:param name="str"/>
<xsl:if test="$str">
			<xsl:variable name="year" select="substring($str,1,4)"/>
			<xsl:variable name="month" select="substring($str,5,2)"/>
			<xsl:variable name="day" select="substring($str,7,2)"/>
			<xsl:variable name="hour" select="substring($str,9,2)"/>
			<xsl:variable name="min" select="substring($str,11,2)"/>
			<xsl:variable name="sec" select="substring($str,13,2)"/>
			<xsl:value-of select="$year"/>-<xsl:value-of select="$month"/>-<xsl:value-of select="$day"/><xsl:text> </xsl:text><xsl:value-of select="$hour"/>:<xsl:value-of select="$min"/>:<xsl:value-of select="$sec"/>.000000</xsl:if>
	</xsl:template>
	
</xsl:stylesheet>
